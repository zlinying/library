    ...
    function readerLogincheckResult(){
    	console.log("回调123！");
        ...
        if (xmlHttp.readyState==4 && xmlHttp.status==200){
			...
			if(document.getElementById("checkinfo").innerHTML == ""){
				window.location.href="reader/main.jsp"; 
			}
        }
    }
    
    function goToRegist(){
    	window.location.href="readerRegist.jsp"; 
    }
    ...
    function readerLogincheck(){
    	var usernameinput = document.getElementById("usernameinput").value;
		var passwordinput = document.getElementById("passwordinput").value;
        createXMLHttpRequest();   //调用创建XMLHttpRequest对象的方法
        xmlHttp.onreadystatechange = readerLogincheckResult;   //设置回调函数
        var url="ReaderLogin?action=login&username=" + usernameinput + "&password=" + passwordinput;
        xmlHttp.open("POST",url,true);      //向服务器端发送请求
        xmlHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=utf8");
        xmlHttp.send(null);
    }

...
public class IODao extends DBConnect{
	...
	public void updateBorrowDay(String borrowTime,int days) {
		try {
            Connection conn = super.getConnection();
            PreparedStatement pst = null;
            String sql = "UPDATE iolog SET borrowday=?,complete=2 WHERE borrowtime=?";
            pst = conn.prepareStatement(sql);
            pst.setInt(1, days);
            pst.setString(2, borrowTime);
            pst.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
	...
	public int getBorrowDay(String borrowTime) {
		try {
            Connection conn = super.getConnection();
            PreparedStatement pst = null;
            ResultSet rs = null;
            String sql = "select borrowday from iolog WHERE borrowtime='"+borrowTime+"'";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            int m = 0;
            while(rs.next()) {
            	m++;
            	return rs.getInt("borrowday");
            }
	...
	public void updateBorrow(String id, String borrowtime) {
		try {
            Connection conn = super.getConnection();
            PreparedStatement pst = null;
            String sql = "update iolog set complete=1 where readerid=? and borrowtime='"+borrowtime+"'";
            pst = conn.prepareStatement(sql);
            pst.setString(1, id);
            pst.executeUpdate();
